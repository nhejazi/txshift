#' 1-TML Estimate of the Effect of a Continuous Treatment (OLD)
#'
...
#'           type = "response")
#' }
#' tmle00 <- tmle1_shift(Y = Y, A = A, W = W, Qn = Qn.0, gn = gn.0, delta = 2,
...
tmle1_shift <- function(Y, A, W,
  ...
  ini.out <- f_iter2(Qn = Qn, gn = gn, gn0d = NULL, prev_sum = 0, first = TRUE,
  ...
  # iterative procedure
  while (abs(ini.out$eps) > tol & iter <= 1) {
    iter = iter + 1
    new.out <- f_iter2(Qn = ini.out$Qn, gn = ini.out$gn, gn0d = gn0d,
                       prev_sum = ini.out$prev_sum, first = FALSE, h_int = h_int,
                       W = W, A = A, A_val = A_val, Y = Y, delta = delta)
    ini.out <- new.out
  }
  Qnd <- t(sapply(seq_len(nrow(W)), function(i) ini.out$Qn(A_val + delta,
                                                           W[i, ])))
