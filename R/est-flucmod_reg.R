...
fit_fluc <- function(Y,
    ...
    Qn_noshift_star_pred <- fitted(mod_fluc)
    ...
    if (method == "standard") {
          Qn_shift_star_in <- as.data.frame(cbind(qlogis(Qn_scaled$upshift),
                                                         Hn$shift))
          ...
          Qn_shift_star_in <- as.data.frame(qlogis(Qn_scaled$upshift))
