...
```{r}
library(shifttx)
tmle_shift_glm <- tmle_shifttx(W = W, A = A, Y = Y, delta = 0.5,
                               g_fit_args = list(nbins = 20,
                                                 bin_method = "dhist",
                                                 bin_estimator =
                                                   speedglmR6$new(),
                                                 parfit = FALSE),
                               Q_fit_args = list(fit_method = "glm",
                                                 glm_formula = "Y ~ ."),
...
### Interlude: Constructing Super Learners
...
# SL learners to be used for the propensity score fit
lrn1_g <- Lrnr_condensier$new(nbins = 35, bin_estimator = lrn1,
                              bin_method = "equal.len")
lrn2_g <- Lrnr_condensier$new(nbins = 25, bin_estimator = lrn2,
                              bin_method = "equal.len")
lrn3_g <- Lrnr_condensier$new(nbins = 35, bin_estimator = lrn3,
                              bin_method = "equal.len")
sl_lrn_g <- Lrnr_sl$new(learners = list(lrn1_g, lrn2_g, lrn3_g),
...
```{r}
tmle_shift_sl <- tmle_shifttx(W = W, A = A, Y = Y, delta = 0.5,
                              g_fit_args = list(nbins = 20,
                                                bin_method = "dhist",
                                                bin_estimator =
                                                  speedglmR6$new(),
...
### Statistical Inference with Targeted Maximum Likelihood Estimation
...
### Incorporating Inverse Probability of Censoring Weights into TML Estimation
...
library(dtplyr)
no_cens_prob <- 0.8
C <- rbinom(n_obs, 1, no_cens_prob)
delta = 0.5

# compute the inverse weights
ipc_weights <- est_ipcw(V = as.data.frame(cbind(W, A)), Delta = C)

# reduce data to only those observations that are not censored
data_O_nocens <- as.data.table(cbind(W, A, Y, C)) %>%
  dplyr::filter(C == 1)

# fit the TML estimator in a stepwise fashion
## 1) propensity score
g_fit_args = list(nbins = 20, bin_method = "dhist",
                  bin_estimator = condensier::speedglmR6$new(), parfit = FALSE)
gn_estim_in <- list(A = data_O_nocens$A, W = data_O_nocens$W, delta = delta)
gn_estim_args <- unlist(list(gn_estim_in, g_fit_args), recursive = FALSE)
gn_estim <- do.call(est_g, gn_estim_args)

## 2) outcome regression
# Q_fit_args = list(fit_method = "sl", glm_formula = "Y ~ .",
#                   sl_learners = c("mean", "glm_fast"), sl_metalearner = "nnls")
...
#     
# ## 3) "clever" covariate estimation
# Hn_estim <- est_Hn(gn = gn_estim)
# 
# ## 4) submodel fluctuation
# fitted_fluc_mod <- fit_fluc(Y = Y, Qn_scaled = Qn_estim, Hn = Hn_estim,
#                             method = fluc_method)
# 
# ## 5) efficient influence function computation
# tmle_eif_out <- tmle_eif(Y = Y, Hn = Hn_estim, fluc_fit_out = fitted_fluc_mod,
#                          tol_eif = eif_tol)
```
