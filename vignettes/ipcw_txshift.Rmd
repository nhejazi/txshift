...
__Note:__ In all examples below, the argument `eif_reg_spec` is set to `FALSE`
(whereas the default is `TRUE`) when invoking an IPCW-TMLE. This is done in an
...
argument to `FALSE` fits this EIF nuisance regression with a generalized linear
model (GLM; an assumption-laden parametric form) while the default value of
`TRUE` fits the nuisance regression with the highly adaptive lasso (HAL; a
recently developed nonparametric estimator) via the [`hal9001` R
package](https://github.com/jeremyrcoyle/hal9001). In practice, we recommend
...
<!--Please consult [INSERT REF] for details on potential benefits this approach may have.-->
...
```{r}
tmle_sl_ineffic <- tmle_txshift(W = W, A = A, Y = Y,
                                C = C, V = c("W", "Y"),
                                delta = delta,
                                fluc_method = "standard",
                                ipcw_fit_args = list(fit_type = "sl",
...
separately (i.e., outside of the `tmle_txshift` wrapper function), instead
...
tmle_shift_spec <- tmle_txshift(W = W, A = A, Y = Y, delta = delta,
                                C = C, V = c("W", "Y"),
                                fluc_method = "standard",
