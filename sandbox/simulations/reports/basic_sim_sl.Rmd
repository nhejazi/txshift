---
title: "Basic Simulations for Shift TMLEs with SL"
...
# simulation
simple_sim_sl1 <- foreach(in_samp = seq_along(n_obs), .combine = rbind) %do% {
...
knitr::kable(simple_sim_sl1, format = "markdown",
...
# simulation
simple_sim_sl2 <- foreach(in_samp = seq_along(n_obs), .combine = rbind) %do% {
  # set sample size from foreach loop
  samp <- n_obs[in_samp]
  ...
  sim_results <- foreach(j = seq_len(n_sim), .combine = rbind) %dopar% {
    ...
    # run the TMLE procedure
    out <- simple_shifttx_sim_sl(iter = j, samp = samp, delta = delta_shift,
                                 n_w = n_w, w1_prob = w_win_prob,
                                 a1_mean = a1_mean, a0_mean = a0_mean,
                                 censoring = TRUE)
  }
...
knitr::kable(simple_sim_sl2, format = "markdown",
             caption = "Simulation statistics for shift TMLE w/ SL")
```
